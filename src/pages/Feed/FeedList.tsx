...
const FeedItem = ({ feed, onCommentClick, onLikeClick }: FeedItemProps) => {
	...
	const handlePrevRootFeedImage = () => {
		...
		const imageUrls = feed.rootFeed?.imageUrls || feed.parentFeed?.imageUrls || [];
			...
			<div className="flex items-center justify-between p-4 border-b border-gray-100">
				<div className="flex items-center gap-3">
					<div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
						<img src={feed.profileImage || userProfile} className="w-10 h-10 rounded-full" alt="프로필" />
					</div>
					<div>
							...
							<>
								...
								<div className="bg-gray-100 border-b border-gray-200 p-3">
									...
									<div className="flex items-center gap-2.5">
										<div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center">
											<img src={feed.parentFeed.profileImage || userProfile} className="w-8 h-8 rounded-full" alt="프로필" />
										</div>
										<div>
								...
								<div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm mx-3 mb-3">
									...
									<div className="flex items-center justify-between p-4 border-b border-gray-100">
										<div className="flex items-center gap-3">
											<div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
												<img src={feed.rootFeed.profileImage || userProfile} className="w-10 h-10 rounded-full" alt="프로필" />
											</div>
											<div>
											...
											<div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
												{feed.rootFeed.imageUrls.map((_, index) => (
													<button
														...
														className={`w-1.5 h-1.5 rounded-full ${index === rootFeedImageIndex ? 'bg-white' : 'bg-white bg-opacity-50'
															}`}
													/>
												))}
											</div>
								...
								<div className="bg-gray-100 p-3">
									...
									<div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
										...
										<div className="p-4 border-b border-gray-100">
											<div className="flex items-center gap-3">
												<div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
													<img src={feed.parentFeed.profileImage || userProfile} className="w-10 h-10 rounded-full" alt="프로필" />
												</div>
												<div>
													<div className="text-sm font-medium">{feed.parentFeed.nickname}</div>
													<div className="text-xs text-gray-500">
														{formatDate(feed.parentFeed.created)}
													</div>
												</div>
	...
	useEffect(() => {
		const fetchFeeds = async () => {
			try {
				...
				const rawFeeds = response.data?.data || response.data?.content || response.data || [];
